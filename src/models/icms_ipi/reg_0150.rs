...
use crate::schemas::reg_0150::dsl as schema;
use crate::schemas::reg_0150::table;
...
#[diesel(table_name = crate::schemas::reg_0150::dsl)]
pub struct Reg0150 {
    pub id: i32,
...
impl Model for Reg0150 {
    ...
    fn get(file_id: i32, parent_id: Option<i32>, conn: &mut SqliteConnection) -> Result<Vec<Reg0150>, Error> {
        if let Some(id) = parent_id {
            Ok(table
                .filter(schema::file_id.eq(&file_id))
                .filter(schema::parent_id.eq(&id))
                .select(Reg0150::as_select())
                .load(conn)?)
        } else {
            Ok(table
                .filter(schema::file_id.eq(&file_id))
...
impl fmt::Display for Reg0150 {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        self.display_format(f)
    }
}

impl_display_fields!(Reg0150, [reg, cod_part, nome, cod_pais, cnpj, cpf, ie, cod_mun, suframa, endereco, num, compl, bairro]);
register_model!(Reg0150, "0150");
