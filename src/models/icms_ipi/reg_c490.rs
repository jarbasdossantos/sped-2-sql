...
use crate::schemas::reg_c490::dsl as schema;
use crate::schemas::reg_c490::table;
...
#[diesel(table_name = crate::schemas::reg_c490::dsl)]
pub struct RegC490 {
    pub id: i32,
...
impl Model for RegC490 {
    ...
    fn get(file_id: i32, parent_id: Option<i32>, conn: &mut SqliteConnection) -> Result<Vec<RegC490>, Error> {
        if let Some(id) = parent_id {
            Ok(table
                .filter(schema::file_id.eq(&file_id))
                .filter(schema::parent_id.eq(&id))
                .select(RegC490::as_select())
                .load(conn)?)
        } else {
            Ok(table
                .filter(schema::file_id.eq(&file_id))
...
impl fmt::Display for RegC490 {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        self.display_format(f)
    }
}

impl_display_fields!(RegC490, [reg, cst_icms, cfop, aliq_icms, vl_opr, vl_bc_icms, vl_icms, cod_obs]);
register_model!(RegC490, "c490");
