...
use crate::schemas::reg_c190::dsl as schema;
use crate::schemas::reg_c190::table;
...
#[diesel(table_name = crate::schemas::reg_c190::dsl)]
pub struct RegC190 {
    pub id: i32,
...
impl Model for RegC190 {
    fn new(
        ...
        RegC190 {
            ...
            vl_ipi: get_field(&fields, 11),
            cod_obs: get_field(&fields, 12),
        }
    }

    fn get(file_id: i32, parent_id: Option<i32>, conn: &mut SqliteConnection) -> Result<Vec<RegC190>, Error> {
        if let Some(id) = parent_id {
            Ok(table
                .filter(schema::file_id.eq(&file_id))
                .filter(schema::parent_id.eq(&id))
                .select(RegC190::as_select())
                .load(conn)?)
        } else {
            Ok(table
                .filter(schema::file_id.eq(&file_id))
...
impl fmt::Display for RegC190 {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        self.display_format(f)
    }
}

impl_display_fields!(RegC190, [reg, cst_icms, cfop, aliq_icms, vl_opr, vl_bc_icms, vl_icms, vl_bc_icms_st, vl_icms_st, vl_red_bc, vl_ipi, cod_obs]);
register_model!(RegC190, "c190");
