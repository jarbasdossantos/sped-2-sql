...
use crate::schemas::reg_1923::dsl as schema;
use crate::schemas::reg_1923::table;
...
#[diesel(table_name = crate::schemas::reg_1923::dsl)]
pub struct Reg1923 {
    pub id: i32,
...
impl Model for Reg1923 {
    ...
    fn get(file_id: i32, parent_id: Option<i32>, conn: &mut SqliteConnection) -> Result<Vec<Reg1923>, Error> {
        if let Some(id) = parent_id {
            Ok(table
                .filter(schema::file_id.eq(&file_id))
                .filter(schema::parent_id.eq(&id))
                .select(Reg1923::as_select())
                .load(conn)?)
        } else {
            Ok(table
                .filter(schema::file_id.eq(&file_id))
...
impl fmt::Display for Reg1923 {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        self.display_format(f)
    }
}

impl_display_fields!(Reg1923, [reg, cod_part, cod_mod, ser, sub, num_doc, dt_doc, cod_item, vl_aj_item]);
register_model!(Reg1923, "1923");
