...
use crate::schemas::reg_c425::dsl as schema;
use crate::schemas::reg_c425::table;
...
#[diesel(table_name = crate::schemas::reg_c425::dsl)]
pub struct RegC425 {
    pub id: i32,
...
impl Model for RegC425 {
    ...
    fn get(file_id: i32, parent_id: Option<i32>, conn: &mut SqliteConnection) -> Result<Vec<RegC425>, Error> {
        if let Some(id) = parent_id {
            Ok(table
                .filter(schema::file_id.eq(&file_id))
                .filter(schema::parent_id.eq(&id))
                .select(RegC425::as_select())
                .load(conn)?)
        } else {
            Ok(table
                .filter(schema::file_id.eq(&file_id))
...
impl fmt::Display for RegC425 {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        self.display_format(f)
    }
}

impl_display_fields!(RegC425, [reg, cod_item, qtd, unid, vl_item, vl_pis, vl_cofins]);
register_model!(RegC425, "c425");
