...
use crate::schemas::reg_d400::dsl as schema;
use crate::schemas::reg_d400::table;
...
#[diesel(table_name = crate::schemas::reg_d400::dsl)]
pub struct RegD400 {
    pub id: i32,
...
impl Model for RegD400 {
    fn new(
        ...
        RegD400 {
            ...
            vl_bc_icms: get_field(&fields, 12),
    ...
    fn get(file_id: i32, parent_id: Option<i32>, conn: &mut SqliteConnection) -> Result<Vec<RegD400>, Error> {
        if let Some(id) = parent_id {
            Ok(table
                .filter(schema::file_id.eq(&file_id))
                .filter(schema::parent_id.eq(&id))
                .select(RegD400::as_select())
                .load(conn)?)
        } else {
            Ok(table
                .filter(schema::file_id.eq(&file_id))
...
impl fmt::Display for RegD400 {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        self.display_format(f)
    }
}

impl_display_fields!(RegD400, [reg, cod_part, cod_mod, cod_sit, ser, sub, num_doc, dt_doc, vl_doc, vl_desc, vl_serv, vl_bc_icms, vl_icms, vl_pis, vl_cofins, cod_cta]);
register_model!(RegD400, "d400");
