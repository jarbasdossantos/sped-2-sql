...
use crate::schemas::reg_1510::dsl as schema;
use crate::schemas::reg_1510::table;
...
#[diesel(table_name = crate::schemas::reg_1510::dsl)]
pub struct Reg1510 {
    pub id: i32,
...
impl Model for Reg1510 {
    fn new(
        ...
        Reg1510 {
            ...
            vl_cofis: get_field(&fields, 20),
            cod_cta: get_field(&fields, 21),
        }
    }

    fn get(file_id: i32, parent_id: Option<i32>, conn: &mut SqliteConnection) -> Result<Vec<Reg1510>, Error> {
        if let Some(id) = parent_id {
            Ok(table
                .filter(schema::file_id.eq(&file_id))
                .filter(schema::parent_id.eq(&id))
                .select(Reg1510::as_select())
                .load(conn)?)
        } else {
            Ok(table
                .filter(schema::file_id.eq(&file_id))
...
impl fmt::Display for Reg1510 {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        self.display_format(f)
    }
}

impl_display_fields!(Reg1510, [reg, num_item, cod_item, cod_class, qtd, unid, vl_item, vl_desc, cst_icms, cfop, vl_bc_icms, aliq_icms, vl_icms, vl_bc_icms_st, aliq_st, vl_icms_st, ind_rec, cod_part, vl_pis, vl_cofis, cod_cta]);
register_model!(Reg1510, "1510");
