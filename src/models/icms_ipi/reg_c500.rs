...
use crate::schemas::reg_c500::dsl as schema;
use crate::schemas::reg_c500::table;
...
#[diesel(table_name = crate::schemas::reg_c500::dsl)]
pub struct RegC500 {
    pub id: i32,
...
impl Model for RegC500 {
    fn new(
        ...
        RegC500 {
            ...
            vl_cofins: get_field(&fields, 25),
            tp_ligacao: get_field(&fields, 26),
            cod_grupo_tensao: get_field(&fields, 27),
        }
    }

    fn get(file_id: i32, parent_id: Option<i32>, conn: &mut SqliteConnection) -> Result<Vec<RegC500>, Error> {
        if let Some(id) = parent_id {
            Ok(table
                .filter(schema::file_id.eq(&file_id))
                .filter(schema::parent_id.eq(&id))
                .select(RegC500::as_select())
                .load(conn)?)
        } else {
            Ok(table
                .filter(schema::file_id.eq(&file_id))
...
impl fmt::Display for RegC500 {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        self.display_format(f)
    }
}

impl_display_fields!(RegC500, [reg, ind_oper, ind_emit, cod_part, cod_mod, cod_sit, ser, sub, cod_cons, num_doc, dt_doc, dt_e_s, vl_doc, vl_desc, vl_forn, vl_serv_nt, vl_terc, vl_da, vl_bc_icms, vl_icms, vl_bc_icms_st, vl_icms_st, cod_inf, vl_pis, vl_cofins, tp_ligacao, cod_grupo_tensao]);
register_model!(RegC500, "c500");
