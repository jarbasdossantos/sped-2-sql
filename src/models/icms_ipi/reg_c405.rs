...
use crate::schemas::reg_c405::dsl as schema;
use crate::schemas::reg_c405::table;
...
#[diesel(table_name = crate::schemas::reg_c405::dsl)]
pub struct RegC405 {
    pub id: i32,
...
impl Model for RegC405 {
    ...
    fn get(file_id: i32, parent_id: Option<i32>, conn: &mut SqliteConnection) -> Result<Vec<RegC405>, Error> {
        if let Some(id) = parent_id {
            Ok(table
                .filter(schema::file_id.eq(&file_id))
                .filter(schema::parent_id.eq(&id))
                .select(RegC405::as_select())
                .load(conn)?)
        } else {
            Ok(table
                .filter(schema::file_id.eq(&file_id))
...
impl fmt::Display for RegC405 {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        self.display_format(f)
    }
}

impl_display_fields!(RegC405, [reg, dt_doc, cro, crz, num_coo_fin, gt_fin, vl_brt]);
register_model!(RegC405, "c405");
