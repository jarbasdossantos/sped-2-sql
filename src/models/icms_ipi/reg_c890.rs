...
use crate::schemas::reg_c890::dsl as schema;
use crate::schemas::reg_c890::table;
...
#[diesel(table_name = crate::schemas::reg_c890::dsl)]
pub struct RegC890 {
    pub id: i32,
...
impl Model for RegC890 {
    ...
    fn get(file_id: i32, parent_id: Option<i32>, conn: &mut SqliteConnection) -> Result<Vec<RegC890>, Error> {
        if let Some(id) = parent_id {
            Ok(table
                .filter(schema::file_id.eq(&file_id))
                .filter(schema::parent_id.eq(&id))
                .select(RegC890::as_select())
                .load(conn)?)
        } else {
            Ok(table
                .filter(schema::file_id.eq(&file_id))
...
impl fmt::Display for RegC890 {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        self.display_format(f)
    }
}

impl_display_fields!(RegC890, [reg, cst_icms, cfop, aliq_icms, vl_opr, vl_bc_icms, vl_icms, cod_obs]);
register_model!(RegC890, "c890");
