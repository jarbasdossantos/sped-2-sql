...
use crate::schemas::reg_e113::dsl as schema;
use crate::schemas::reg_e113::table;
...
#[diesel(table_name = crate::schemas::reg_e113::dsl)]
pub struct RegE113 {
    pub id: i32,
...
impl Model for RegE113 {
    ...
    fn get(file_id: i32, parent_id: Option<i32>, conn: &mut SqliteConnection) -> Result<Vec<RegE113>, Error> {
        if let Some(id) = parent_id {
            Ok(table
                .filter(schema::file_id.eq(&file_id))
                .filter(schema::parent_id.eq(&id))
                .select(RegE113::as_select())
                .load(conn)?)
        } else {
            Ok(table
                .filter(schema::file_id.eq(&file_id))
...
impl fmt::Display for RegE113 {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        self.display_format(f)
    }
}

impl_display_fields!(RegE113, [reg, cod_part, cod_mod, ser, sub, num_doc, dt_doc, cod_item, vl_aj_item]);
register_model!(RegE113, "e113");
