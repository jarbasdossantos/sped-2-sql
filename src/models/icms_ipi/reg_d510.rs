...
use crate::schemas::reg_d510::dsl as schema;
use crate::schemas::reg_d510::table;
...
#[diesel(table_name = crate::schemas::reg_d510::dsl)]
pub struct RegD510 {
    pub id: i32,
...
impl Model for RegD510 {
    fn new(
        ...
        RegD510 {
            ...
            ind_rec: get_field(&fields, 16),
            cod_part: get_field(&fields, 17),
            vl_pis: get_field(&fields, 18),
            vl_cofins: get_field(&fields, 19),
            cod_cta: get_field(&fields, 20),
        }
    }

    fn get(file_id: i32, parent_id: Option<i32>, conn: &mut SqliteConnection) -> Result<Vec<RegD510>, Error> {
        if let Some(id) = parent_id {
            Ok(table
                .filter(schema::file_id.eq(&file_id))
                .filter(schema::parent_id.eq(&id))
                .select(RegD510::as_select())
                .load(conn)?)
        } else {
            Ok(table
                .filter(schema::file_id.eq(&file_id))
...
impl fmt::Display for RegD510 {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        self.display_format(f)
    }
}

impl_display_fields!(RegD510, [reg, num_item, cod_item, cod_class, qtd, unid, vl_item, vl_desc, cst_icms, cfop, vl_bc_icms, aliq_icms, vl_icms, vl_bc_icms_st, vl_icms_st, ind_rec, cod_part, vl_pis, vl_cofins, cod_cta]);
register_model!(RegD510, "d510");
