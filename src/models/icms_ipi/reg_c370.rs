...
use crate::schemas::reg_c370::dsl as schema;
use crate::schemas::reg_c370::table;
...
#[diesel(table_name = crate::schemas::reg_c370::dsl)]
pub struct RegC370 {
    pub id: i32,
...
impl Model for RegC370 {
    ...
    fn get(file_id: i32, parent_id: Option<i32>, conn: &mut SqliteConnection) -> Result<Vec<RegC370>, Error> {
        if let Some(id) = parent_id {
            Ok(table
                .filter(schema::file_id.eq(&file_id))
                .filter(schema::parent_id.eq(&id))
                .select(RegC370::as_select())
                .load(conn)?)
        } else {
            Ok(table
                .filter(schema::file_id.eq(&file_id))
...
impl fmt::Display for RegC370 {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        self.display_format(f)
    }
}

impl_display_fields!(RegC370, [reg, num_item, cod_item, qtd, unid, vl_item, vl_desc]);
register_model!(RegC370, "c370");
