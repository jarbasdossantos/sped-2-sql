...
use crate::schemas::reg_d101::dsl as schema;
use crate::schemas::reg_d101::table;
...
#[diesel(table_name = crate::schemas::reg_d101::dsl)]
pub struct RegD101 {
    pub id: i32,
...
impl Model for RegD101 {
    ...
    fn get(file_id: i32, parent_id: Option<i32>, conn: &mut SqliteConnection) -> Result<Vec<RegD101>, Error> {
        if let Some(id) = parent_id {
            Ok(table
                .filter(schema::file_id.eq(&file_id))
                .filter(schema::parent_id.eq(&id))
                .select(RegD101::as_select())
                .load(conn)?)
        } else {
            Ok(table
                .filter(schema::file_id.eq(&file_id))
...
impl fmt::Display for RegD101 {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        self.display_format(f)
    }
}

impl_display_fields!(RegD101, [reg, ind_nat_frt, vl_item, cst_pis, nat_bc_cred, vl_bc_pis, aliq_pis, vl_pis, cod_cta]);
register_model!(RegD101, "d101");
