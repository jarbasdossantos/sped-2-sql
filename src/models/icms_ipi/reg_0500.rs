...
use crate::schemas::reg_0500::dsl as schema;
use crate::schemas::reg_0500::table;
...
#[diesel(table_name = crate::schemas::reg_0500::dsl)]
pub struct Reg0500 {
    pub id: i32,
...
impl Model for Reg0500 {
    ...
    fn get(file_id: i32, parent_id: Option<i32>, conn: &mut SqliteConnection) -> Result<Vec<Reg0500>, Error> {
        if let Some(id) = parent_id {
            Ok(table
                .filter(schema::file_id.eq(&file_id))
                .filter(schema::parent_id.eq(&id))
                .select(Reg0500::as_select())
                .load(conn)?)
        } else {
            Ok(table
                .filter(schema::file_id.eq(&file_id))
...
impl fmt::Display for Reg0500 {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        self.display_format(f)
    }
}

impl_display_fields!(Reg0500, [reg, dt_alt, cod_nat_cc, ind_cta, nivel, cod_cta, nome_cta]);
register_model!(Reg0500, "0500");
