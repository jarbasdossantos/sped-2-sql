...
use crate::schemas::reg_c590::dsl as schema;
use crate::schemas::reg_c590::table;
...
#[diesel(table_name = crate::schemas::reg_c590::dsl)]
pub struct RegC590 {
    pub id: i32,
...
impl Model for RegC590 {
    ...
    fn get(file_id: i32, parent_id: Option<i32>, conn: &mut SqliteConnection) -> Result<Vec<RegC590>, Error> {
        if let Some(id) = parent_id {
            Ok(table
                .filter(schema::file_id.eq(&file_id))
                .filter(schema::parent_id.eq(&id))
                .select(RegC590::as_select())
                .load(conn)?)
        } else {
            Ok(table
                .filter(schema::file_id.eq(&file_id))
...
impl fmt::Display for RegC590 {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        self.display_format(f)
    }
}

impl_display_fields!(RegC590, [reg, cst_icms, cfop, aliq_icms, vl_opr, vl_bc_icms, vl_icms, vl_bc_icms_st, vl_icms_st, vl_red_bc, cod_obs]);
register_model!(RegC590, "c590");
