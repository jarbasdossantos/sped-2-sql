...
use crate::schemas::reg_c197::dsl as schema;
use crate::schemas::reg_c197::table;
...
#[diesel(table_name = crate::schemas::reg_c197::dsl)]
pub struct RegC197 {
    pub id: i32,
...
impl Model for RegC197 {
    ...
    fn get(file_id: i32, parent_id: Option<i32>, conn: &mut SqliteConnection) -> Result<Vec<RegC197>, Error> {
        if let Some(id) = parent_id {
            Ok(table
                .filter(schema::file_id.eq(&file_id))
                .filter(schema::parent_id.eq(&id))
                .select(RegC197::as_select())
                .load(conn)?)
        } else {
            Ok(table
                .filter(schema::file_id.eq(&file_id))
...
impl fmt::Display for RegC197 {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        self.display_format(f)
    }
}

impl_display_fields!(RegC197, [reg, cod_aj, descr_compl_aj, cod_item, vl_bc_icms, aliq_icms, vl_icms, vl_outros]);
register_model!(RegC197, "c197");
