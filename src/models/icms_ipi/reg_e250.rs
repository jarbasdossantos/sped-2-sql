...
use crate::schemas::reg_e250::dsl as schema;
use crate::schemas::reg_e250::table;
...
#[diesel(table_name = crate::schemas::reg_e250::dsl)]
pub struct RegE250 {
    pub id: i32,
...
impl Model for RegE250 {
    ...
    fn get(file_id: i32, parent_id: Option<i32>, conn: &mut SqliteConnection) -> Result<Vec<RegE250>, Error> {
        if let Some(id) = parent_id {
            Ok(table
                .filter(schema::file_id.eq(&file_id))
                .filter(schema::parent_id.eq(&id))
                .select(RegE250::as_select())
                .load(conn)?)
        } else {
            Ok(table
                .filter(schema::file_id.eq(&file_id))
...
impl fmt::Display for RegE250 {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        self.display_format(f)
    }
}

impl_display_fields!(RegE250, [reg, cod_or, vl_or, dt_vcto, cod_rec, num_proc, ind_proc, proc_, txt_compl, mes_ref]);
register_model!(RegE250, "e250");
