...
pub struct Reg0175 {
    ...
    pub nr_campo: Option<String>,
    pub cont_ant: Option<String>,
}

#[async_trait]
impl Model for Reg0175 {
    fn new(
        fields: Vec<&str>,
        new_id: Option<i32>,
    ...
    fn get(file_id: i32, parent_id: Option<i32>, conn: &mut SqliteConnection) -> Result<Vec<Reg0175>, Error> {
        if let Some(id) = parent_id {
            Ok(table
                .filter(schema::file_id.eq(&file_id))
                .filter(schema::parent_id.eq(&id))
                .select(Reg0175::as_select())
                .load(conn)?)
        } else {
            Ok(table
                .filter(schema::file_id.eq(&file_id))
...
impl fmt::Display for Reg0175 {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        self.display_format(f)
    }
}

impl_display_fields!(Reg0175, [reg, dt_alt, nr_campo, cont_ant]);
register_model!(Reg0175, "0175");
