...
use crate::schemas::reg_d355::dsl as schema;
use crate::schemas::reg_d355::table;
...
#[diesel(table_name = crate::schemas::reg_d355::dsl)]
pub struct RegD355 {
    pub id: i32,
...
impl Model for RegD355 {
    ...
    fn get(file_id: i32, parent_id: Option<i32>, conn: &mut SqliteConnection) -> Result<Vec<RegD355>, Error> {
        if let Some(id) = parent_id {
            Ok(table
                .filter(schema::file_id.eq(&file_id))
                .filter(schema::parent_id.eq(&id))
                .select(RegD355::as_select())
                .load(conn)?)
        } else {
            Ok(table
                .filter(schema::file_id.eq(&file_id))
...
impl fmt::Display for RegD355 {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        self.display_format(f)
    }
}

impl_display_fields!(RegD355, [reg, dt_doc, cro, crz, num_coo_fin, gt_fin, vl_brt]);
register_model!(RegD355, "d355");
