...
use crate::schemas::efd_c870::dsl as schema;
use crate::schemas::efd_c870::table;
...
#[diesel(table_name = crate::schemas::efd_c870::dsl)]
pub struct EfdC870 {
    pub id: i32,
...
impl Model for EfdC870 {
    ...
    fn get(file_id: i32, parent_id: Option<i32>, conn: &mut SqliteConnection) -> Result<Vec<EfdC870>, Error> {
        if let Some(id) = parent_id {
            Ok(table
                .filter(schema::file_id.eq(&file_id))
                .filter(schema::parent_id.eq(&id))
                .select(EfdC870::as_select())
                .load(conn)?)
        } else {
            Ok(table
                .filter(schema::file_id.eq(&file_id))
...
impl fmt::Display for EfdC870 {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        self.display_format(f)
    }
}

impl_display_fields!(EfdC870, [reg, cod_item, cfop, vl_item, vl_desc, cst_pis, vl_bc_pis, aliq_pis, vl_pis, cst_cofins, vl_bc_cofins, aliq_cofins, vl_cofins, cod_cta]);
register_model!(EfdC870, "c870");
