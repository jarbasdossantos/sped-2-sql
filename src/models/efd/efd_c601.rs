...
use crate::schemas::efd_c601::efd_c601::dsl as schema;
use crate::schemas::efd_c601::efd_c601::table;
...
#[diesel(table_name = crate::schemas::efd_c601::efd_c601::dsl)]
pub struct EfdC601 {
    pub id: i32,
...
impl Model for EfdC601 {
    ...
    async fn get(file_id: i32, parent_id: Option<i32>) -> Result<Vec<EfdC601>, Error> {
        Ok(table
            .filter(schema::file_id.eq(&file_id))
            .filter(schema::parent_id.eq(parent_id.expect("Invalid parent id")))
            .select(EfdC601::as_select())
            .load(&mut DB_POOL
                .get().unwrap())?)
    }
...
impl fmt::Display for EfdC601 {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        self.display_format(f)
    }
}

impl_display_fields!(EfdC601, [reg, cst_pis, vl_item, vl_bc_pis, aliq_pis, vl_pis, cod_cta]);
register_model!(EfdC601, "c601");
