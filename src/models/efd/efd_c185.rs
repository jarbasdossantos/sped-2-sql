...
use crate::schemas::efd_c185::efd_c185::dsl as schema;
use crate::schemas::efd_c185::efd_c185::table;
...
#[diesel(table_name = crate::schemas::efd_c185::efd_c185::dsl)]
pub struct EfdC185 {
    pub id: i32,
...
impl Model for EfdC185 {
    ...
    async fn get(file_id: i32, parent_id: Option<i32>) -> Result<Vec<EfdC185>, Error> {
        Ok(table
            .filter(schema::file_id.eq(&file_id))
            .filter(schema::parent_id.eq(&parent_id.expect("Invalid parent id")))
            .select(EfdC185::as_select())
            .load(&mut DB_POOL
                .get().unwrap())?)
    }
...
impl fmt::Display for EfdC185 {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        self.display_format(f)
    }
}

impl_display_fields!(EfdC185, [reg, cst_cofins, cfop, vl_item, vl_desc, vl_bc_cofins, aliq_cofins, quant_bc_cofins, aliq_cofins_quant, vl_cofins, cod_cta]);
register_model!(EfdC185, "c185");
