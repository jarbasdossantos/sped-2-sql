...
use crate::schemas::efd_d501::efd_d501::dsl as schema;
use crate::schemas::efd_d501::efd_d501::table;
...
#[diesel(table_name = crate::schemas::efd_d501::efd_d501::dsl)]
pub struct EfdD501 {
    pub id: i32,
...
impl Model for EfdD501 {
    ...
    async fn get(file_id: i32, parent_id: Option<i32>) -> Result<Vec<EfdD501>, Error> {
        Ok(table
            .filter(schema::file_id.eq(&file_id))
            .filter(schema::parent_id.eq(&parent_id.expect("Invalid parent id")))
            .select(EfdD501::as_select())
            .load(&mut DB_POOL
                .get().unwrap())?)
    }
...
impl fmt::Display for EfdD501 {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        self.display_format(f)
    }
}

impl_display_fields!(EfdD501, [reg, cst_pis, vl_item, nat_bc_cred, vl_bc_pis, aliq_pis, vl_pis, cod_cta]);
register_model!(EfdD501, "d501");
