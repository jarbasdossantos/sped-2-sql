...
use crate::schemas::efd_m611::efd_m611::dsl as schema;
use crate::schemas::efd_m611::efd_m611::table;
...
#[diesel(table_name = crate::schemas::efd_m611::efd_m611::dsl)]
pub struct EfdM611 {
    pub id: i32,
...
impl Model for EfdM611 {
    ...
    async fn get(file_id: i32, parent_id: Option<i32>) -> Result<Vec<EfdM611>, Error> {
        Ok(table
            .filter(schema::file_id.eq(&file_id))
            .filter(schema::parent_id.eq(parent_id.expect("Invalid parent id")))
            .select(EfdM611::as_select())
            .load(&mut DB_POOL
                .get().unwrap())?)
    }
...
impl fmt::Display for EfdM611 {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        self.display_format(f)
    }
}

impl_display_fields!(EfdM611, [reg, ind_tip_coop, vl_bc_cont_ant_exc_coop, vl_exc_coop_ger, vl_exc_esp_coop, vl_bc_cont]);
register_model!(EfdM611, "m611");
