...
use crate::schemas::efd_m110::efd_m110::dsl as schema;
use crate::schemas::efd_m110::efd_m110::table;
...
#[diesel(table_name = crate::schemas::efd_m110::efd_m110::dsl)]
pub struct EfdM110 {
    pub id: i32,
...
impl Model for EfdM110 {
    ...
    async fn get(file_id: i32, parent_id: Option<i32>) -> Result<Vec<EfdM110>, Error> {
        Ok(table
            .filter(schema::file_id.eq(&file_id))
            .filter(schema::parent_id.eq(&parent_id.expect("Invalid parent id")))
            .select(EfdM110::as_select())
            .load(&mut DB_POOL
                .get().unwrap())?)
    }
...
impl fmt::Display for EfdM110 {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        self.display_format(f)
    }
}

impl_display_fields!(EfdM110, [reg, ind_aj, vl_aj, cod_aj, num_doc, descr_aj, dt_ref]);
register_model!(EfdM110, "m110");
