...
use crate::schemas::efd_c501::efd_c501::dsl as schema;
use crate::schemas::efd_c501::efd_c501::table;
...
#[diesel(table_name = crate::schemas::efd_c501::efd_c501::dsl)]
pub struct EfdC501 {
    pub id: i32,
...
impl Model for EfdC501 {
    ...
    async fn get(file_id: i32, parent_id: Option<i32>) -> Result<Vec<EfdC501>, Error> {
        Ok(table
            .filter(schema::file_id.eq(&file_id))
            .filter(schema::parent_id.eq(parent_id.expect("Invalid parent id")))
            .select(EfdC501::as_select())
            .load(&mut DB_POOL
                .get().unwrap())?)
    }
...
impl fmt::Display for EfdC501 {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        self.display_format(f)
    }
}

impl_display_fields!(EfdC501, [reg, cst_pis, vl_item, nat_bc_cred, vl_bc_pis, aliq_pis, vl_pis, cod_cta]);
register_model!(EfdC501, "c501");
