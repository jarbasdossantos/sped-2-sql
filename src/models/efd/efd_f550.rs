...
use crate::schemas::efd_f550::efd_f550::dsl as schema;
use crate::schemas::efd_f550::efd_f550::table;
...
#[diesel(table_name = crate::schemas::efd_f550::efd_f550::dsl)]
pub struct EfdF550 {
    pub id: i32,
...
impl Model for EfdF550 {
    ...
    async fn get(file_id: i32, parent_id: Option<i32>) -> Result<Vec<EfdF550>, Error> {
        Ok(table
            .filter(schema::file_id.eq(&file_id))
            .filter(schema::parent_id.eq(&parent_id.expect("Invalid parent id")))
            .select(EfdF550::as_select())
            .load(&mut DB_POOL
                .get().unwrap())?)
    }
...
impl fmt::Display for EfdF550 {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        self.display_format(f)
    }
}

impl_display_fields!(EfdF550, [reg, vl_rec_comp, cst_pis, vl_desc_pis, vl_bc_pis, aliq_pis, vl_pis, cst_cofins, vl_desc_cofins, vl_bc_cofins, aliq_cofins, vl_cofins, cod_mod, cfop, cod_cta, info_compl]);
register_model!(EfdF550, "f550");
