...
use crate::schemas::efd_c505::efd_c505::dsl as schema;
use crate::schemas::efd_c505::efd_c505::table;
...
#[diesel(table_name = crate::schemas::efd_c505::efd_c505::dsl)]
pub struct EfdC505 {
    pub id: i32,
...
impl Model for EfdC505 {
    ...
    async fn get(file_id: i32, parent_id: Option<i32>) -> Result<Vec<EfdC505>, Error> {
        Ok(table
            .filter(schema::file_id.eq(&file_id))
            .filter(schema::parent_id.eq(&parent_id.expect("Invalid parent id")))
            .select(EfdC505::as_select())
            .load(&mut DB_POOL
                .get().unwrap())?)
    }
...
impl fmt::Display for EfdC505 {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        self.display_format(f)
    }
}

impl_display_fields!(EfdC505, [reg, cst_cofins, vl_item, nat_bc_cred, vl_bc_cofins, aliq_cofins, vl_cofins, cod_cta]);
register_model!(EfdC505, "c505");
