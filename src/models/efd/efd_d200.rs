...
use crate::schemas::efd_d200::efd_d200::dsl as schema;
use crate::schemas::efd_d200::efd_d200::table;
...
#[diesel(table_name = crate::schemas::efd_d200::efd_d200::dsl)]
pub struct EfdD200 {
    pub id: i32,
...
impl Model for EfdD200 {
    fn new(
        ...
        dt_ref: get_field(&fields, 9),
        vl_doc: get_field(&fields, 10),
        vl_desc: get_field(&fields, 11),
        }
    }

    async fn get(file_id: i32, parent_id: Option<i32>) -> Result<Vec<EfdD200>, Error> {
        Ok(table
            .filter(schema::file_id.eq(&file_id))
            .filter(schema::parent_id.eq(parent_id.expect("Invalid parent id")))
            .select(EfdD200::as_select())
            .load(&mut DB_POOL
                .get().unwrap())?)
    }
...
impl fmt::Display for EfdD200 {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        self.display_format(f)
    }
}

impl_display_fields!(EfdD200, [reg, cod_mod, cod_sit, ser, sub, num_doc_ini, num_doc_fin, cfop, dt_ref, vl_doc, vl_desc]);
register_model!(EfdD200, "d200");
