...
use crate::schemas::efd_c120::efd_c120::dsl as schema;
use crate::schemas::efd_c120::efd_c120::table;
...
#[diesel(table_name = crate::schemas::efd_c120::efd_c120::dsl)]
pub struct EfdC120 {
    pub id: i32,
...
impl Model for EfdC120 {
    ...
    async fn get(file_id: i32, parent_id: Option<i32>) -> Result<Vec<EfdC120>, Error> {
        Ok(table
            .filter(schema::file_id.eq(&file_id))
            .filter(schema::parent_id.eq(parent_id.expect("Invalid parent id")))
            .select(EfdC120::as_select())
            .load(&mut DB_POOL
                .get().unwrap())?)
    }
...
impl fmt::Display for EfdC120 {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        self.display_format(f)
    }
}

impl_display_fields!(EfdC120, [reg, cod_doc_imp, num_doc_imp, vl_pis_imp, vl_cofins_imp, num_acdraw]);
register_model!(EfdC120, "c120");
