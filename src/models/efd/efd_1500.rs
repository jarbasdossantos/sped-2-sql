...
use crate::schemas::efd_1500::efd_1500::dsl as schema;
use crate::schemas::efd_1500::efd_1500::table;
...
#[diesel(table_name = crate::schemas::efd_1500::efd_1500::dsl)]
pub struct Efd1500 {
    pub id: i32,
...
impl Model for Efd1500 {
    ...
    async fn get(file_id: i32, parent_id: Option<i32>) -> Result<Vec<Efd1500>, Error> {
        Ok(table
            .filter(schema::file_id.eq(&file_id))
            .filter(schema::parent_id.eq(&parent_id.expect("Invalid parent id")))
            .select(Efd1500::as_select())
            .load(&mut DB_POOL
                .get().unwrap())?)
    }
...
impl fmt::Display for Efd1500 {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        self.display_format(f)
    }
}

impl_display_fields!(Efd1500, [reg, per_apu_cred, orig_cred, cnpj_suc, cod_cred, vl_cred_apu, vl_cred_ext_apu, vl_tot_cred_apu, vl_cred_desc_pa_ant, vl_cred_per_pa_ant, vl_cred_dcomp_pa_ant, sd_cred_disp_efd, vl_cred_desc_efd, vl_cred_per_efd, vl_cred_dcomp_efd, vl_cred_trans, vl_cred_out, sld_cred_fim]);
register_model!(Efd1500, "1500");
