...
use crate::schemas::efd_d205::efd_d205::dsl as schema;
use crate::schemas::efd_d205::efd_d205::table;
...
#[diesel(table_name = crate::schemas::efd_d205::efd_d205::dsl)]
pub struct EfdD205 {
    pub id: i32,
...
impl Model for EfdD205 {
    fn new(
        ...
        vl_cofins: get_field(&fields, 6),
        cod_cta: get_field(&fields, 7),
        }
    }

    async fn get(file_id: i32, parent_id: Option<i32>) -> Result<Vec<EfdD205>, Error> {
        Ok(table
            .filter(schema::file_id.eq(&file_id))
            .filter(schema::parent_id.eq(parent_id.expect("Invalid parent id")))
            .select(EfdD205::as_select())
            .load(&mut DB_POOL
                .get().unwrap())?)
    }
...
impl fmt::Display for EfdD205 {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        self.display_format(f)
    }
}

impl_display_fields!(EfdD205, [reg, cst_cofins, vl_item, vl_bc_cofins, aliq_cofins, vl_cofins, cod_cta]);
register_model!(EfdD205, "d205");
