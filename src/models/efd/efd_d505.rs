...
use crate::schemas::efd_d505::efd_d505::dsl as schema;
use crate::schemas::efd_d505::efd_d505::table;
...
#[diesel(table_name = crate::schemas::efd_d505::efd_d505::dsl)]
pub struct EfdD505 {
    pub id: i32,
...
impl Model for EfdD505 {
    fn new(
        ...
        EfdD505 {
            ...
            nat_bc_cred: get_field(&fields, 4),
            vl_bc_cofins: get_field(&fields, 5),
            aliq_cofins: get_field(&fields, 6),
            vl_cofins: get_field(&fields, 7),
            cod_cta: get_field(&fields, 8),
        }
    }

    async fn get(file_id: i32, parent_id: Option<i32>) -> Result<Vec<EfdD505>, Error> {
        Ok(table
            .filter(schema::file_id.eq(&file_id))
            .filter(schema::parent_id.eq(&parent_id.expect("Invalid parent id")))
            .select(EfdD505::as_select())
            .load(&mut DB_POOL
                .get().unwrap())?)
    }
...
impl fmt::Display for EfdD505 {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        self.display_format(f)
    }
}

impl_display_fields!(EfdD505, [reg, cst_cofins, vl_item, nat_bc_cred, vl_bc_cofins, aliq_cofins, vl_cofins, cod_cta]);
register_model!(EfdD505, "d505");
